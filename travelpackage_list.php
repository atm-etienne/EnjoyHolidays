...
/**
 *   	\file       travelpackage_list.php
 *		\ingroup    enjoyholidays
 *		\brief      List page for travelpackage
 */
...
require_once __DIR__.'/class/travelpackage.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('enjoyholidays', 'travelpackage', 'write');
	$permissiontodelete = $user->hasRight('enjoyholidays', 'travelpackage', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("enjoyholidays")) {
	accessforbidden('Module enjoyholidays not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'TravelPackage';
	$objectlabel = 'TravelPackage';
	$uploaddir = $conf->enjoyholidays->dir_output;
...
$title = $langs->trans("TravelPackages");
//$help_url = "EN:Module_TravelPackage|FR:Module_TravelPackage_FR|ES:Módulo_TravelPackage";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/enjoyholidays/travelpackage_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-enjoyholidays page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/enjoyholidays/travelpackage_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendTravelPackageRef";
$modelmail = "travelpackage";
$objecttmp = new TravelPackage($db);
